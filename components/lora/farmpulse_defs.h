...
#define MIN_MQTT_frame_SIZE 34      // Size " # PASS GID NID FC AC DATA $ = 34 bytes"
...
typedef enum {
    ...
    PARAM_ALARM_MASK            = 0x08, // Alarm Mask
    PARAM_DEVICE_ID             = 0x09, // Change Node ID
    PARAM_SIGNATURE             = 0x0A, // EEPROM Signature
    PARAM_RSSI_THRESH           = 0x0B, // RSSI Filter Threshold
    PARAM_HB_INTERVAL           = 0x0C, // Heartbeat reporting period
    PARAM_METER_DATA_REQ        = 0x0D, // Force Telemetry Transmission
    PARAM_REBOOT                = 0x0E  // System Reboot
} param_id_t;
...
typedef struct {
    ...
    uint8_t param_id;        // Parameter ID (e.g., PARAM_LORA_MOTOR_CTRL)
...
extern addr_t device_id; // 0=OFF, 1=ON
extern uint8_t motor_action; // 0=OFF, 1=ON
extern uint16_t alarm_reg1; 
extern uint16_t alarm_reg2; 
extern uint16_t pwr_ON_cnt;
extern uint32_t dev_UUID;
